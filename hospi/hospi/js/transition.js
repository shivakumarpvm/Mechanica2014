...
var PageTransitions = (function () {
    ...
    function init() {
        ...
        // Get all the .page div.
        $('.page').each( function() {
            var $page = $(this);
            $page.data('originalClassList', $page.attr('class'));
        });

        // Get all the .wrapper div which is the parent for all div
        $('.wrapper').each( function() {
            var $wrapperDiv = $(this);
            $wrapperDiv.data('current', 0);
            $wrapperDiv.data('isAnimating', false);
            $wrapperDiv.children('.page').eq(startPageIndex).addClass('page-current');
        });

        // Adding click event to .trigger
    ...
    // All trigger click event calls this function
    ...
    function Animate($pageTrigger) {
        ...
        // This will get the trigger elements parent wrapper div
        var $pageWrapper = $pageTrigger.closest('.wrapper');
        var currentPageIndex = $pageWrapper.data('current'), tempPageIndex,
            $pages = $pageWrapper.children('div.page'),
        ...
        // check if 'data-goto' value is greater than total pages inside 'wrapper'
        if (!(pagesCount < gotoPage)) {
            ...
            if (tempPageIndex != currentPageIndex) {
                ...
                var $nextPage = $pages.eq(currentPageIndex).addClass('page-current');
    ...
    function resetPage($nextPage, $currentPage) {
        ...
        $nextPage.attr('class', $nextPage.data('originalClassList') + ' page-current');
    }
